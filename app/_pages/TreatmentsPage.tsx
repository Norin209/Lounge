...
const heroImage = "https://images.unsplash.com/photo-1600334129128-685c5582fd35?q=80&w=2000&auto=format&fit=crop";
const treatmentPlaceholder = "https://images.unsplash.com/photo-1544161515-4ab6ce6db874?q=80&w=800&auto=format&fit=crop";
...
const TreatmentsPage = () => {
  ...
  const handleAddToBag = (item: ServiceItem) => {
    addToBag({
      ...
      image: item.image || treatmentPlaceholder 
    });
  };
    ...
    <div className="bg-white min-h-screen pb-24">
      <section className="relative h-[50vh] w-full flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6">
          <h1 className="text-4xl md:text-5xl font-playfair text-white mb-4 uppercase tracking-[0.3em]">Our Full Menu</h1>
          <div className="w-12 h-px bg-white/50 mx-auto mb-4" />
          <p className="text-white/70 text-[10px] tracking-[0.4em] uppercase font-light">Luxury Cambodian Wellness</p>
        </div>
      </section>
      ...
      <nav className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-gray-100 shadow-sm">
        <div className="relative max-w-7xl mx-auto px-6 py-6 group">
          ...
          >
            {tabs.map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`whitespace-nowrap text-[10px] md:text-[11px] font-bold tracking-[0.2em] uppercase transition-all duration-300 border-b-2 pb-1 shrink-0 ${activeTab === tab ? "text-black border-black" : "text-gray-400 border-transparent hover:text-black"}`}>
                {tab}
              </button>
            ))}
          </div>
        ...
        <div ref={scrollContainerRef} className="flex flex-nowrap overflow-x-auto gap-6 snap-x snap-mandatory pb-8 hide-scrollbar md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 md:gap-x-10 md:gap-y-20 md:overflow-visible md:pb-0 md:snap-none">
          {filteredItems.length > 0 ? (
            filteredItems.map((item) => {
              ...
              const displayImage = item.image || treatmentPlaceholder;
              ...
              return (
                <div key={item.id} className="group shrink-0 w-72 snap-start md:w-auto md:snap-align-none flex flex-col h-full">
                  <div className="relative aspect-4/5 overflow-hidden mb-6 bg-gray-50 shadow-sm shrink-0">
                    ...
                    <Image src={displayImage} alt={item.name} fill className="object-cover transition-transform duration-1000 group-hover:scale-105" />
                    ...
                    <div className="absolute inset-0 bg-black/10 md:bg-black/20 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 flex items-end pb-4 justify-center px-4">
                      {isInBag ? (
                        <Link href="/book" className="w-full bg-black text-white py-3 text-[10px] font-bold tracking-[0.2em] uppercase shadow-lg text-center hover:bg-gray-900 transition-colors">View Bag</Link>
                      ) : (
                        <button onClick={() => handleAddToBag(item)} className="w-full bg-white text-black py-3 text-[10px] font-bold tracking-[0.2em] uppercase shadow-lg hover:bg-gray-100 transition-colors">
                          Add to Bag
                        </button>
                      )}
                    </div>
