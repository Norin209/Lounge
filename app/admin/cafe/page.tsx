...
export default function AdminCafe() {
  ...
  const startEditing = (item: any) => { 
    ...
    setNewItem({ 
      ...
      isMonthlyPromo: item.isMonthlyPromo || false, 
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editingId) {
      await updateDoc(doc(db, "cafe", editingId), newItem);
    } else {
      await addDoc(collection(db, "cafe"), { ...newItem, createdAt: serverTimestamp() });
    }
    cancelEditing();
    fetchItems();
  };
      ...
      <div className={`p-6 rounded-xl border ${editingId ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-200 shadow-sm'}`}>
        ...
        <form onSubmit={handleFormSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
           ...
           <div className="md:col-span-3">
             ...
             <input value={newItem.name || ''} onChange={e=>setNewItem({...newItem, name: e.target.value})} className={inputStyle} placeholder="e.g. Latte" />
           </div>
           ...
           <div className="md:col-span-3">
             ...
             <select value={newItem.category || 'coffee'} onChange={e=>setNewItem({...newItem, category: e.target.value})} className={selectStyle}>
           ...
           <div className="md:col-span-9">
             ...
             <input value={newItem.description || ''} onChange={e=>setNewItem({...newItem, description: e.target.value})} className={inputStyle} placeholder="Describe the item..." />
           </div>
              ...
              {filteredItems.map((s) => (
                <tr key={s.id} className={`transition-all ${s.isPaused ? 'bg-gray-50 opacity-60 grayscale-50' : 'hover:bg-gray-50'}`}>
                  ...
                  <td className="px-6 py-4">
                    ...
                    <p className="text-[10px] text-gray-400 uppercase">{s.category}</p>
                  </td>
